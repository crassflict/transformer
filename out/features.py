...
Pré-requis (dans ton venv ET dans requirements.txt) :
    pandas
    numpy
    scipy
    pykalman
...
# colonnes que le modèle utilisera
...
def _compute_rsi(close: pd.Series, period: int = 14) -> pd.Series:
    ...
    # moyennes mobiles exponentielles pour plus de réactivité
...
def _apply_savgol(close: pd.Series, window: int, poly: int) -> pd.Series:
    if len(close) < window:
        # pas assez de points : on renvoie des NaN
...
def add_features(df: pd.DataFrame, cfg: FeatureConfig | None = None) -> pd.DataFrame:
    ...
    # Kalman & Savitzky–Golay sur le prix de clôture
    ...
    # Label : direction du prochain close
    future_close = close.shift(-1)
    future_ret = (future_close / close) - 1.0
    df[LABEL_COLUMN] = (future_ret > 0).astype(int)

    # On enlève les lignes avec NaN (warm-up EMA/RSI etc. + dernière ligne sans label)
